...
public class Order {
    ...
    public void addItem(String item, double price) {
        if (price <= 0) throw new IllegalArgumentException("Invalid price: must be greater than 0");
        if (item == null || item.isEmpty()) throw new IllegalArgumentException("Invalid item: cannot be empty");
        items.add(item);
        prices.add(price);
    }
